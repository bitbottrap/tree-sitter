...
pub fn build_tables(
    ...
    // Build option (keyword-create-trace): when
    // TREE_SITTER_KEYWORD_CREATE_TRACE=1, time the keyword-identification pass
    // (which decides which tokens join the keyword DFA) and report it through
    // build_lex_table's KWCREATE block. Unset => zero overhead.
    let kw_create_trace =
