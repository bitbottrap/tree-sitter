...
fn compute_keyword_depths(
    ...
    // sound. This replaces the old "separator must be pure whitespace" test,
    // which declined grammars (tcl, fsharp, purescript) whose separator merely
    // *looks* unsafe but can never actually fire at a word start. When the two
    // sets DO intersect (agda's `\`), bucketing is still sound for words that
    // don't begin with an intersection character; those that do are routed to
    // the full DFA by a first-character peek in the dispatcher.
...
/// exactly its length: `build_bucket_dfa` builds the subset DFA as before
/// (compact loops for cyclic keywords), then a (state, byte-length) pair DFS
...
/// `PATHPULSE$…` rides every length >= 13 up to the bucket length cap as
/// before. A symbol with NO computable coverage (an accept state a
...
/// inflate the generated code per distinct length. Corpus keywords top out at
/// 19 bytes (verilog), so only verilog's two 18/19-byte literals move to the
/// residual.
...
fn build_keyword_buckets(
    ...
    //   a weight-1 self-loop rides every length >= its minimum as before.
...
pub fn build_lex_table(
    ...
    // Build option (eval/keyword-dfs): when TREE_SITTER_KEYWORD_CREATE_TRACE=1,
    // time the keyword-DFS *creation* phases and print them to stderr as
    // `KWCREATE <phase> <ns> <detail>` lines. The creation harness
    // (eval/keyword-dfs/collect_creation.py) drives `tree-sitter generate`
    // with this set and parses those lines; unset => zero overhead, and
    ...
    fn add_state_for_tokens(&mut self, tokens: &TokenSet) -> LexStateId {
        ...
        if is_new {
            debug!(
                ...
                tokens
                    .iter()
                    // EOF and external tokens have no lexical-grammar variable;
                    // indexing one used to panic under --log (e.g. yaml, whose
                    // entry state contains only external tokens).
